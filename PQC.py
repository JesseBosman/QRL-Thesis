...
def generate_circuit(qubits, n_layers, n_inputs, RxCnot=False):
    ...
    if RxCnot:
        for l in range(n_layers):
            ...
            circuit += cirq.Circuit(
                one_qubit_rx(q, params[l, i]) for i, q in enumerate(qubits)
            )
            circuit += entangling_layer_CNOT(qubits)
            # Encoding layer
            circuit += cirq.Circuit(
                cirq.rx(inputs[l, i])(qubits[i]) for i in range(n_inputs)
            )

        # Last varitional layer
        circuit += cirq.Circuit(
            one_qubit_rx(q, params[n_layers, i]) for i, q in enumerate(qubits)
        )
...
class ReUploadingPQC(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        circuit, theta_symbols, input_symbols = generate_circuit(
            qubits, n_layers, n_inputs, RxCnot=RxCnot
        )
...
class ObservableWeights(tf.keras.layers.Layer):
    def __init__(self, output_dim):
        ...
        self.w = tf.Variable(
            initial_value=np.random.normal(scale=0.01, size=output_dim),
            dtype="float32",
            trainable=True,
            name="obs-weights",
        )
...
def generate_model_policy(n_qubits, n_layers, n_actions, n_inputs, beta, RxCnot=False):
    ...
    input_tensor = tf.keras.Input(
        shape=(n_inputs,), dtype=tf.dtypes.float32, name="input"
    )
    re_uploading_pqc = ReUploadingPQC(
        qubits, n_layers, n_inputs, observables, RxCnot=RxCnot
    )([input_tensor])
...
def reinforce_update(states, actions, returns, model, ws, optimizers, batch_size, eta):
    ...
    with tf.GradientTape() as tape:
        ...
        logits = model(states)
        entropy_loss = -1 * tf.math.reduce_sum(
            tf.math.multiply(logits, tf.math.log(logits)), axis=1
        )
        p_actions = tf.gather_nd(logits, actions)
        log_probs = tf.math.log(p_actions)
        loss = (
            tf.math.reduce_sum(-log_probs * returns) - eta * entropy_loss
        ) / batch_size
